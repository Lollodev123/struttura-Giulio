...
**Variante**: $C_p = 0{,}9$ richiesto (per atterraggio lento). Quale $\alpha$ per profilo NACA 2412? E per NACA 0012?
...
| Profilo | $\alpha_0$ | $\alpha$ richiesto per $C_p = 0{,}9$ | Margine fino allo stallo |
|---|---|---|---|
...
- Il profilo asimmetrico A ha bisogno di **2° in meno** per generare lo stesso $C_p$ → fusoliera più orizzontale in atterraggio → migliore visibilità sul muso, comfort passeggeri.
...
- **Ala**: deve generare portanza in volo livellato → asimmetrico (curvato), $C_p$ "gratis" a $\alpha = 0$
